...
public enum AttributeModifierOperation {
    ...
    MULTIPLY_TOTAL_ADDITIVE(Phase.TOTAL, 0, (base, total, mod) -> new Result(base, total * (total * mod))),
    MULTIPLY_TOTAL_MULTIPLICATIVE(Phase.TOTAL, 1, (base, total, mod) -> new Result(base, total * (1.0 + mod))),
    STANDARD_MULTIPLY_TOTAL(Phase.TOTAL, 2, (base, total, mod) -> new Result(base, total * mod)),
    STANDARD_DIVIDE_TOTAL(Phase.TOTAL, 3, (base, total, mod) -> new Result(base, mod == 0 ? total : total / mod)),
    MIN_TOTAL(Phase.TOTAL, 4, (base, total, mod) -> new Result(base, Math.max(total, mod))),
    MAX_TOTAL(Phase.TOTAL, 5, (base, total, mod) -> new Result(base, Math.min(total, mod))),
    SET_TOTAL(Phase.TOTAL, 6, (base, total, mod) -> new Result(base, mod));

    public enum Phase { BASE, TOTAL }

    public record Result(double base, double total) {}

    @FunctionalInterface
    public interface Apply { Result apply(double base, double total, double modifier); }
    ...
    public static final Codec<AttributeModifierOperation> CODEC = Codec.STRING.flatXmap(
        s -> {
            AttributeModifierOperation v = ALIASES.get(s);
            if (v != null) return com.mojang.serialization.DataResult.success(v);
            for (AttributeModifierOperation op : values()) {
                if (op.serializedName().equals(s)) return com.mojang.serialization.DataResult.success(op);
            }
    ...
    public net.minecraft.world.entity.ai.attributes.AttributeModifier.Operation vanillaOperation() {
        return switch (this) {
            case ADD_BASE_EARLY, ADD_BASE_LATE, SET_BASE -> net.minecraft.world.entity.ai.attributes.AttributeModifier.Operation.ADDITION;
            case MULTIPLY_BASE_ADDITIVE, MULTIPLY_BASE_MULTIPLICATIVE,
                 STANDARD_MULTIPLY_BASE, STANDARD_DIVIDE_BASE,
                 MIN_BASE, MAX_BASE -> net.minecraft.world.entity.ai.attributes.AttributeModifier.Operation.MULTIPLY_BASE;
            case MULTIPLY_TOTAL_ADDITIVE, MULTIPLY_TOTAL_MULTIPLICATIVE,
                 STANDARD_MULTIPLY_TOTAL, STANDARD_DIVIDE_TOTAL,
                 MIN_TOTAL, MAX_TOTAL, SET_TOTAL -> net.minecraft.world.entity.ai.attributes.AttributeModifier.Operation.MULTIPLY_TOTAL;
        };
    }
}
