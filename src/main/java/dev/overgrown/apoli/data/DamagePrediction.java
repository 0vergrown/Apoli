...
public final class DamagePrediction {
    ...
    public static float predict(LivingEntity target, DamageSource source, float amount) {
        ...
        if (source.is(DamageTypeTags.DAMAGES_HELMET) && !target.getItemBySlot(EquipmentSlot.HEAD).isEmpty()) {
            f *= 0.75F;
        }
    ...
    private static float afterArmor(LivingEntity target, @Nullable LivingEntity attacker,
        ...
        return CombatRules.getDamageAfterAbsorb(target, f, source, target.getArmorValue(),
    ...
    private static float afterMagic(LivingEntity target, DamageSource source, float f, Level level) {
        ...
        if (!(level instanceof ServerLevel serverLevel)) return f;
        float protection = EnchantmentHelper.getDamageProtection(serverLevel, target, source);
        return protection > 0.0F ? CombatRules.getDamageAfterMagicAbsorb(f, protection) : f;
    }
