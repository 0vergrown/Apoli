...
public final class RopeClientManager {
    ...
    private static void changeRopeLength(VerletRopeState rope, Minecraft client) {
        ...
        if (delta != 0f) {
            ...
            new RopeChangeLengthC2S(delta).write(buf);
            ClientPlayNetworking.send(RopeChangeLengthC2S.CHANNEL, buf);
        }
    ...
    private static void swing(Minecraft client) {
        ...
        FriendlyByteBuf buf = new FriendlyByteBuf(Unpooled.buffer());
        new RopeSwingC2S(dir.lengthSqr() > 1 ? dir.normalize() : dir).write(buf);
        ClientPlayNetworking.send(RopeSwingC2S.CHANNEL, buf);
    }
