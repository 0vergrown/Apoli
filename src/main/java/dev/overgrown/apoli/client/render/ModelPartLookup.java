...
public final class ModelPartLookup {
    ...
    public static void resolveInto(HumanoidModel<?> model, String normalized, List<ModelPart> parts) {
        ...
        switch (normalized) {
            case "head" -> { parts.add(model.head); parts.add(model.hat); }
            case "hat" -> parts.add(model.hat);
            case "body" -> { parts.add(model.body); if (player != null) parts.add(player.jacket); }
            case "rightarm" -> { parts.add(model.rightArm); if (player != null) parts.add(player.rightSleeve); }
            case "leftarm" -> { parts.add(model.leftArm); if (player != null) parts.add(player.leftSleeve); }
            case "rightleg" -> { parts.add(model.rightLeg); if (player != null) parts.add(player.rightPants); }
            case "leftleg" -> { parts.add(model.leftLeg); if (player != null) parts.add(player.leftPants); }
            case "jacket" -> { if (player != null) parts.add(player.jacket); }
            case "rightsleeve" -> { if (player != null) parts.add(player.rightSleeve); }
            case "leftsleeve" -> { if (player != null) parts.add(player.leftSleeve); }
            case "rightpants" -> { if (player != null) parts.add(player.rightPants); }
            case "leftpants" -> { if (player != null) parts.add(player.leftPants); }
            default -> {}
        }
    }
