...
public class SixArmsPlayerModel<T extends LivingEntity> extends PlayerModel<T> implements ExtraModelParts {
    ...
    private static final float SECOND_ARM_ROLL = (float) Math.toRadians(13.5);
    private static final float SECOND_ARM_DROP = 3.0F;
    private static final float THIRD_ARM_ROLL = (float) Math.toRadians(27.0);
    private static final float THIRD_ARM_DROP = 6.0F;
    ...
    public final ModelPart rightThirdArm;
    public final ModelPart rightThirdSleeve;
    public final ModelPart leftThirdArm;
    public final ModelPart leftThirdSleeve;

    private final List<ModelPart> extraParts;
    private final List<ModelPart> bodyParts;

    public SixArmsPlayerModel(ModelPart root, boolean slim) {
        super(root, slim);
        ...
        this.rightThirdArm = root.getChild("right_third_arm");
        this.rightThirdSleeve = root.getChild("right_third_sleeve");
        this.leftThirdArm = root.getChild("left_third_arm");
        this.leftThirdSleeve = root.getChild("left_third_sleeve");
        this.extraParts = List.of(this.rightSecondArm, this.leftSecondArm,
            this.rightSecondSleeve, this.leftSecondSleeve,
            this.rightThirdArm, this.leftThirdArm,
            this.rightThirdSleeve, this.leftThirdSleeve);
    ...
    public static LayerDefinition createLayer(boolean slim) {
        ...
        float basePivotY = slim ? 2.5F : 2.0F;
        float secondPivotY = basePivotY + SECOND_ARM_DROP;
        float thirdPivotY = basePivotY + THIRD_ARM_DROP;
        addPair(root, "second", leftInset, rightInset, width, secondPivotY, none, sleeve);
        addPair(root, "third", leftInset, rightInset, width, thirdPivotY, none, sleeve);
        return LayerDefinition.create(mesh, 64, 64);
    }

    private static void addPair(PartDefinition root, String ordinal,
                                float leftInset, float rightInset, float width, float pivotY,
                                CubeDeformation none, CubeDeformation sleeve) {
        root.addOrReplaceChild("left_" + ordinal + "_arm", CubeListBuilder.create().texOffs(32, 48)
            .addBox(leftInset, -2.0F, -2.0F, width, 12.0F, 4.0F, none), PartPose.offset(5.0F, pivotY, 0.0F));
        root.addOrReplaceChild("right_" + ordinal + "_arm", CubeListBuilder.create().texOffs(40, 16)
            .addBox(rightInset, -2.0F, -2.0F, width, 12.0F, 4.0F, none), PartPose.offset(-5.0F, pivotY, 0.0F));
        root.addOrReplaceChild("left_" + ordinal + "_sleeve", CubeListBuilder.create().texOffs(48, 48)
            .addBox(leftInset, -2.0F, -2.0F, width, 12.0F, 4.0F, sleeve), PartPose.offset(5.0F, pivotY, 0.0F));
        root.addOrReplaceChild("right_" + ordinal + "_sleeve", CubeListBuilder.create().texOffs(40, 32)
    ...
    public void setupAnim(T entity, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
        ...
        this.rightThirdArm.copyFrom(this.rightArm);
        this.leftThirdArm.copyFrom(this.leftArm);
        this.rightThirdSleeve.copyFrom(this.rightSleeve);
        this.leftThirdSleeve.copyFrom(this.leftSleeve);
        ...
        this.rightThirdArm.zRot -= THIRD_ARM_ROLL;
        this.rightThirdSleeve.zRot -= THIRD_ARM_ROLL;
        this.leftThirdArm.zRot += THIRD_ARM_ROLL;
        this.leftThirdSleeve.zRot += THIRD_ARM_ROLL;
        ...
        this.rightThirdArm.y += THIRD_ARM_DROP;
        this.leftThirdArm.y += THIRD_ARM_DROP;
        this.rightThirdSleeve.y += THIRD_ARM_DROP;
        this.leftThirdSleeve.y += THIRD_ARM_DROP;
    }
    ...
    public void collectExtraParts(String normalized, List<ModelPart> parts) {
        switch (normalized) {
            ...
            case "rightthirdarm", "thirdrightarm" -> {
                parts.add(this.rightThirdArm);
                parts.add(this.rightThirdSleeve);
            }
            case "leftthirdarm", "thirdleftarm" -> {
