...
public final class DynamicTextures {
    ...
    private static @Nullable ResourceLocation skin(Entity subject) {
        return subject instanceof AbstractClientPlayer player ? player.getSkinTextureLocation() : entityTexture(subject);
    }

    private static @Nullable ResourceLocation cape(Entity subject) {
        return subject instanceof AbstractClientPlayer player ? player.getCloakTextureLocation() : null;
    }
    ...
    private static @Nullable ResourceLocation fromPlayerInfo(TextureRef.Kind kind, java.util.UUID uuid) {
        ...
        return switch (kind) {
            case PLAYER, PLAYER_FACE, ENTITY -> info.getSkinLocation();
            case PLAYER_CAPE -> info.getCapeLocation();
            default -> null;
        };
    }
}
