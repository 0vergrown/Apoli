    ...
    public static void onClientTick() {
        ...
        for (ResourceLocation powerId : ClientPowerState.localPowers()) {
            ...
            if (cfg.key().continuous()) {
                trigger = km.isDown();
                while (km.consumeClick()) { /* drain so it doesn't trigger again next tick */ }
            } else {
                trigger = false;
            ...
            new PowerActivationC2S(powerId).write(buf);
            ClientPlayNetworking.send(PowerActivationC2S.CHANNEL, buf);
        }
    }
}
