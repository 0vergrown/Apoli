...
public final class SpeechClient {
    ...
    public static void onJoin() {
        ...
        BiConsumer<String, String> onTranscript = (text, lang) ->
            Minecraft.getInstance().execute(() -> {
                ...
                if (mc.player != null && mc.getConnection() != null && mc.getConnection().hasChannel(SpeechC2S.TYPE)) {
                    PacketDistributor.sendToServer(new SpeechC2S(text, lang == null || lang.isBlank() ? language : lang));
                }
            });
    ...
    private static void startVoskFallback(String language, BiConsumer<String, String> onTranscript) {
        ...
        Path modelDir = Minecraft.getInstance().gameDirectory.toPath().resolve("apoli").resolve("vosk-model");
