    ...
    public static void onJoin() {
        ...
        BiConsumer<String, String> onTranscript = (text, lang) ->
            Minecraft.getInstance().execute(() -> {
                if (Minecraft.getInstance().player != null && ClientPlayNetworking.canSend(SpeechC2S.TYPE)) {
                    ClientPlayNetworking.send(new SpeechC2S(text, lang == null || lang.isBlank() ? language : lang));
                }
            });
