...
public final class SpeechClient {
    ...
    private static Path modelDir() {
        return FabricLoader.getInstance().getGameDir().resolve("apoli").resolve("vosk-model");
    }
    ...
    private static void matchAndSend(Minecraft mc, String text, boolean complete) {
        ...
        if (!ClientPlayNetworking.canSend(SpeechTriggerC2S.CHANNEL)) {
        ...
        PowerLookup.forEachEntry(mc.player, ApoliIds.ACTION_ON_SPEECH, ActionOnSpeechPower.Config.class,
            (powerId, cfg) -> {
                ...
                new SpeechTriggerC2S(powerId).write(buf);
                ClientPlayNetworking.send(SpeechTriggerC2S.CHANNEL, buf);
            });
