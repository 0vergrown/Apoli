...
public final class HardnessBlockCondition implements ConditionType<BlockCtx, HardnessBlockCondition.Cfg> {
    ...
    public boolean test(Cfg cfg, BlockCtx ctx) {
        return cfg.comparison.compare(ctx.state().getDestroySpeed(ctx.level(), ctx.pos()), cfg.compareTo);
    }
}
