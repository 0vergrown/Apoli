...
public final class PassengerCondition implements ConditionType<EntityCtx, PassengerCondition.Cfg> {
    ...
    public boolean test(Cfg cfg, EntityCtx ctx) {
        ...
        for (Entity passenger : target.getPassengers()) {
            ...
            if (cfg.bientityCondition.isPresent()) {
                if (!cfg.bientityCondition.get().test(new BiEntityCtx(livingPassenger, target, ctx.level()))) continue;
            }
            count++;
        }
