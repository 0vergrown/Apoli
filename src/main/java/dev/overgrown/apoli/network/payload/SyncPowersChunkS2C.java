...
public record SyncPowersChunkS2C(int index, int total, byte[] slice) {
    public static final ResourceLocation CHANNEL = Apoli.id("sync_powers_chunk");

    public void write(FriendlyByteBuf buf) {
        buf.writeVarInt(index);
        buf.writeVarInt(total);
        buf.writeByteArray(slice);
    }

    public static SyncPowersChunkS2C read(FriendlyByteBuf buf) {
        return new SyncPowersChunkS2C(buf.readVarInt(), buf.readVarInt(), buf.readByteArray());
    }
