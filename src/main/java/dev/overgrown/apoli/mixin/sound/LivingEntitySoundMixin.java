...
public abstract class LivingEntitySoundMixin {
    ...
    private void apoli$replaceEatSound(Level level, Player except, double x, double y, double z,
    ...
    @WrapOperation(method = "onEquipItem(Lnet/minecraft/world/entity/EquipmentSlot;Lnet/minecraft/world/item/ItemStack;Lnet/minecraft/world/item/ItemStack;)V", at = @At(value = "INVOKE",
    ...
    private void apoli$replaceEquipSound(Level level, Player except, double x, double y, double z,
                                         Holder<SoundEvent> sound, SoundSource source, float volume, float pitch,
                                         long seed, Operation<Void> original) {
        SoundReplacer.emit((Entity) (Object) this, sound.value(), volume, pitch,
            (replaced, newVolume, newPitch) -> original.call(level, except, x, y, z,
                BuiltInRegistries.SOUND_EVENT.wrapAsHolder(replaced), source, newVolume, newPitch, seed));
    }
}
