...
public abstract class LavaVisionMixin {
    ...
    private static float apoli$modifyLavaVisibilityMinWithoutFireResistance(float original, Camera camera) { //v
        if(LavaVisionHandler.getModifier(camera.getEntity()).getB() != 0f) return Math.max(0f, LavaVisionHandler.getModifier(camera.getEntity()).getA());
    ...
    @ModifyExpressionValue(method = "setupFog", at = @At(value = "CONSTANT", args = "floatValue=3.0F", ordinal = 0))
