...
public final class AdvancementAction implements ActionType<EntityCtx, AdvancementAction.Cfg> {
    ...
    public void run(Cfg cfg, EntityCtx ctx) {
        ...
        AdvancementHolder holder = mgr.get(cfg.advancement.get());
        if (holder == null) return;
        PlayerAdvancements progress = player.getAdvancements();
        Set<String> criteria = resolveCriteria(cfg, holder);
        for (String c : criteria) {
            if (cfg.revoke) progress.revoke(holder, c);
            else progress.award(holder, c);
        }
    }

    private static Set<String> resolveCriteria(Cfg cfg, AdvancementHolder holder) {
        if (cfg.criteria.isPresent()) return Set.copyOf(cfg.criteria.get());
        if (cfg.criterion.isPresent()) return Set.of(cfg.criterion.get());
        return holder.value().criteria().keySet();
    }
}
