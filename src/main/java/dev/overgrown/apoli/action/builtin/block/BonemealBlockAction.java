...
public final class BonemealBlockAction implements ActionType<BlockCtx, BonemealBlockAction.Cfg> {
    ...
    public void run(Cfg cfg, BlockCtx ctx) {
        ...
        if (!bonemealable.isValidBonemealTarget(serverLevel, ctx.pos(), state)) return;
        if (!bonemealable.isBonemealSuccess(serverLevel, serverLevel.random, ctx.pos(), state)) return;
