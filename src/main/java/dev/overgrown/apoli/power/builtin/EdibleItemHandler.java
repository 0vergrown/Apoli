...
public final class EdibleItemHandler {
    ...
    private static final ThreadLocal<Slot> ACTIVE_SLOT = ThreadLocal.withInitial(Slot::new);
    ...
    public static int armedDuration(ItemStack stack) {
        Slot slot = ACTIVE_SLOT.get();
        if (slot.config == null || slot.item != stack.getItem()) return -1;
    ...
    private static void arm(LivingEntity entity, ItemStack stack, EdibleItemPower.Config cfg) {
        Slot slot = ACTIVE_SLOT.get();
