...
public record CustomParticleOptions(
    ...
    public static final MapCodec<CustomParticleOptions> CODEC = Codec.mapPair(BODY_CODEC, LOOK_CODEC).xmap(
        pair -> pair.getFirst().withLook(pair.getSecond()),
        options -> Pair.of(options, new Look(options.blend(), options.facing(), options.easing())));
