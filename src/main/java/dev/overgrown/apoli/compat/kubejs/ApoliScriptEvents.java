...
public final class ApoliScriptEvents {
    ...
    private static void bindAction(ApoliScripts.Kind kind, EventHandler handler) {
        ApoliScripts.setEventDispatcher(kind, (id, ctx) -> {
            if (!handler.hasListeners(id)) return false;
            handler.post(new ApoliScriptEventJS(ctx), id);
            return true;
        });
    }

    private static void bindCondition(ApoliScripts.Kind kind, EventHandler handler) {
        ApoliScripts.setEventPredicate(kind, (id, ctx) -> {
            if (!handler.hasListeners(id)) return null;
            EventResult result = handler.post(new ApoliScriptEventJS(ctx), id);
